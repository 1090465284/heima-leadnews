...
public class WmMaterialServiceImpl extends ServiceImpl<WmMaterialMapper, WmMaterial> implements WmMaterialService {
    ...
    public ResponseResult uploadPicture(MultipartFile multipartFile) {
        ...
        String fileId = null;
        try {
            fileId = fileStorageService.uploadImgFile("", fileNames + postfix, multipartFile.getInputStream());
            log.info("上传图片到minio中,图片id:{}", fileId);
        } catch (IOException e) {
            e.printStackTrace();
            log.error("上传图片失败");
        }
    ...
    @Override
    public ResponseResult setCollection(Integer id) {
        if(id == null){
            return ResponseResult.errorResult(AppHttpCodeEnum.PARAM_INVALID);
        }
        WmMaterial wm = getById(id);
        if(wm == null){
            return ResponseResult.errorResult(AppHttpCodeEnum.PARAM_INVALID);
        }
        LambdaUpdateWrapper<WmMaterial> lambdaUpdateWrapper = new LambdaUpdateWrapper<>();
        lambdaUpdateWrapper.eq(WmMaterial::getId, id);
        lambdaUpdateWrapper.set(WmMaterial::getIsCollection, (wm.getIsCollection() + 1) % 2);
    ...
    public ResponseResult delPicture(Integer id) {
        ...
        removeById(id);
        fileStorageService.delete(wm.getUrl());
